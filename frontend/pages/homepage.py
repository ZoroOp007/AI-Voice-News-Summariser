...
BACKEND_URL = "https://localhost:1234"
...
def main():
    ...
    with st.sidebar:
        ...
        st.image("frontend\data\mic-logo-symbol-for-tv-or-news-company-free-vector.jpg", caption=user , width=200)
    ...
    if st.session_state.topics:
        ...
        if st.button("🚀 Generate Summary", disabled=len(st.session_state.topics) == 0):
            if not st.session_state.topics:
                st.erro("Please add at least one topic")
            else:
